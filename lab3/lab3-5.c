...
GLfloat projectionMatrix[] = {
	...
	void init(void)
	{
		...
		LoadTGATextureSimple("rutor.tga", &rutor);
		...
		glUniform1i(glGetUniformLocation(program, "texMask"), 2); // Texture unit 2
		glActiveTexture(GL_TEXTURE3);
		glBindTexture(GL_TEXTURE_2D, rutor);
		glUniform1i(glGetUniformLocation(program, "texRut"), 3); // Texture unit 2
	...
	void display(void)
	{
		...
		glUniform1i(glGetUniformLocation(program, "hasTex"), false);
		//glActiveTexture(GL_TEXTURE1);
		...
		DrawModel(blades,program,"in_Position","in_Normal",NULL);
		...
		printError("display");
			...
			glUniform1i(glGetUniformLocation(program, "hasTex"), true);
		int i;
		for(i = 0; i < 3; i++){
			totalbunny = Mult(Ry(M_PI/2*i),S(3,3,3));
			totalbunny = Mult(T(5+i*6,1.5,5),totalbunny);
